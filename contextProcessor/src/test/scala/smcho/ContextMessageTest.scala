...
class ContextMessageTest extends FunSuite with BeforeAndAfterEach {
  ...
  test ("test simple") {
    ...
    val expected = """[0->0/0.0/g1c0l:g3c1b/0]"""
    assert(c.toString == expected)
  }

  test ("test simple2") {
    val c = ContextMessage(0,5,3.4,nameTypesString,81)
  ...
  test ("test repr") {
    val c = ContextMessage(nameTypesString)
    val expected = """{"host1":0, "host2":0, "time":0.0, "nameTypes":"g1c0l:g3c1b", "size":0}"""
    assert(c.repr == expected)
  }

  test ("test repr 2") {
    val c = ContextMessage(0,5,3.4,nameTypesString,81)
    val expected = """{"host1":0, "host2":5, "time":3.4, "nameTypes":"g1c0l:g3c1b", "size":81}"""
    assert(c.repr == expected)
  }
}
